...
@WebServlet("/NV_thongkeDSDKOnL")
public class NV_thongkeDSDKOnL extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public NV_thongkeDSDKOnL() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.getRequestDispatcher("/WEB-INF/NV-thong-ke-DSDKOnL.jsp").forward(request, response);
	}
		...
		if(flag == 2) {
			...
			if(!dsHVDKyOnl.isEmpty())  {
				if(dsHVDKyOnl.get(0).getError() == null) {
					Gson gson = new Gson();
				    String objectToReturn = gson.toJson(dsHVDKyOnl); 
				    out.write(objectToReturn); 
				    out.flush();
				}
				else {
